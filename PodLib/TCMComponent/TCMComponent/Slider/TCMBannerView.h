...
@protocol TCMBannerViewProtocol <NSObject>

-(void)bannerView:(TCMBannerView*)bannerView clickedOnIndex:(NSInteger)index image:(UIImage*)image;

@end

@interface TCMBannerView : UIView
/**
 图片间有间距  又要有翻页效果～～
...
//clickImageBlock和delegate同时使用，优先delegate
@property(nonatomic, weak) id <TCMBannerViewProtocol> delegate; //协议委托对象
/** 点击中间图片的回调 */
...
@property (nonatomic,strong) TCMBannerView *scrollView;
@property (nonatomic,strong) TCMBannerView *scrollView1;
#pragma mark - 滚动图
_scrollView = [TCMBannerView initWithFrame:CGRectMake(0, 100, KScreenWidth, 150) imageSpacing:10 imageWidth:KScreenWidth - 50];
...
#pragma mark - 普通滚动图
_scrollView1 = [TCMBannerView initWithFrame:CGRectMake(0, 300, KScreenWidth, 150) imageSpacing:0 imageWidth:KScreenWidth];
