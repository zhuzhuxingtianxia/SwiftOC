...
// from:https://github.com/LvJianfeng/LLCycleScrollView
...
/// CyclePageViewDelegate
@objc public protocol CyclePageViewDelegate: class {
    @objc func cycleScrollView(_ cycleScrollView: CyclePageView, didSelectItemIndex index: NSInteger)
    @objc optional func cycleScrollView(_ cycleScrollView: CyclePageView, scrollTo index: NSInteger)
}
...
open class CyclePageView: UIView {
    ...
    /// 加载状态图
    fileprivate var placeHolderViewImage: UIImage! = UIImage(named: "llplaceholder.png", in: Bundle(for: CyclePageView.self), compatibleWith: nil)
    
    /// 空数据占位图
    fileprivate var coverViewImage: UIImage! = UIImage(named: "llplaceholder.png", in: Bundle(for: CyclePageView.self), compatibleWith: nil)
...
extension CyclePageView {
    ...
    open class func llCycleScrollViewWithFrame(_ frame: CGRect, imageURLPaths: Array<String>? = [], titles:Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> CyclePageView {
        let llcycleScrollView: CyclePageView = CyclePageView.init(frame: frame)
        // Nil
    ...
    ///   - didSelectItemAtIndex: Closure
    /// - Returns: LLCycleScrollView
    open class func llCycleScrollViewWithTitles(frame: CGRect, backImage: UIImage? = nil, titles: Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> CyclePageView {
        let llcycleScrollView: CyclePageView = CyclePageView.init(frame: frame)
        // Nil
        llcycleScrollView.titles = []
    ...
    ///   - didSelectItemAtIndex: Closure
    /// - Returns: LLCycleScrollView
    open class func llCycleScrollViewWithArrow(_ frame: CGRect, arrowLRImages: [UIImage], arrowLRFrame: [CGRect]? = nil, imageURLPaths: Array<String>? = [], titles:Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> CyclePageView {
        let llcycleScrollView: CyclePageView = CyclePageView.init(frame: frame)
        // Nil
        llcycleScrollView.imagePaths = []
        llcycleScrollView.titles = []
...
extension CyclePageView {
    ...
    private func setupMainView() {
        ...
        collectionView.register(CyclePageViewCell.self, forCellWithReuseIdentifier: identifier)
    ...
    override open func layoutSubviews() {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system || customPageControlStyle == .image {
            if pageControlPosition == .center {
                pageControl?.frame = CGRect.init(x: 0, y: frame.height-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
            }else{
                let pointSize = pageControl?.size(forNumberOfPages: self.imagePaths.count)
                if pageControlPosition == .left {
                    pageControl?.frame = CGRect.init(x: -(UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: frame.height-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }else{
                    pageControl?.frame = CGRect.init(x: (UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: frame.height-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }
            }
        }else{
            var y = frame.height-pageControlBottom
...
extension CyclePageView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: CyclePageViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as! CyclePageViewCell
        // Setting
        cell.titleFont = font
...
extension CyclePageView: UIScrollViewDelegate {
