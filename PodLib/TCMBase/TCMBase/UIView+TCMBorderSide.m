...
- (void)setBorderType:(NSInteger)borderType {
    objc_setAssociatedObject(self, @selector(borderType), [NSNumber numberWithInteger:borderType], OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self borderForColor:[UIColor colorWithCGColor:self.layer.borderColor] borderWidth:self.layer.borderWidth borderType:borderType];
}
- (NSInteger)borderType {
    return [objc_getAssociatedObject(self, @selector(borderType)) integerValue];
